...
class TestListingFilters:
    ...
    def test_filter_by_min_price_only(self):
        ...
        assert resp.status_code == 200
        prices = [float(item["price"]) for item in resp.json()["results"]]
    ...
    def test_filter_by_location_icontains(self):
        ...
        assert resp.status_code == 200
        assert len(resp.json()["results"]) == 2  # Both Brooklyn listings
        # All lowercase query
    ...
    def test_filter_by_category_iexact(self):
        ...
        assert resp.status_code == 200
        assert len(resp.json()["results"]) == 2  # Both Electronics listings
        # All uppercase query
        ...
        assert resp.status_code == 200
        assert len(resp.json()["results"]) == 0  # No match since it's exact match
    ...
    def test_ordering_by_price(self):
        ...
        prices = [float(i["price"]) for i in resp.json()["results"]]
    ...
    def test_ordering_by_created_at(self):
        ...
        ids = [i["listing_id"] for i in resp.json()["results"]]
